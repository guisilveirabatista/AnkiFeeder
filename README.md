...
flashcard, complete with its translation and example sentences in both English
and Dutch — generated by Claude.

- **Input:** a markdown note in your Obsidian vault, one word (or phrase) per line.
  Each word may be English **or** Dutch — the language is detected automatically.
- **Output:** a "Basic" card in your chosen Anki deck — **Front** = the word,
  **Back** = its translation plus a 🇬🇧 English and 🇳🇱 Dutch example sentence.
...
…) and overrides per-note fields like `note_path` and `deck_name`. A single
`watch` then runs every feed concurrently in one process:
...
```json
{
  ...
  "feeds": [
    ...
    { "note_path": "~/Obsidian/dutch/expressions.md", "deck_name": "Dutch::Expressions" },
    { "note_path": "~/Obsidian/french/vocab.md",      "deck_name": "French::Vocabulary" }
  ]
}
```
